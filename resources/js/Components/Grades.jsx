...
export default function Grades({ data }) {
    ...
    useEffect(() => {
        ...
        if (selectedGradeLevel) {
            filtered = filtered.filter(
                (item) => item.grade_level === selectedGradeLevel
            );
        }
        ...
        if (selectedGradeType) {
            filtered = filtered.map((item) => {
                ...
                if (
                    selectedGradeType === "calificativa" &&
                    typeof grade === "number"
                ) {
                    item.grade = gradeToLetter(grade);
                } else if (
                    selectedGradeType === "cuantitativa" &&
                    typeof grade === "string"
                ) {
        ...
        if (selectedTimePeriod) {
            filtered = filtered.filter(
                (item) => item.time_period === selectedTimePeriod
            );
        }
        ...
        <div className="flex flex-wrap gap-2 align-items-center justify-between">
            <div className="flex">
                <Button
                    icon="pi pi-plus"
                    label="Nuevo"
                    onClick={() => setNewGradeDialog(true)}
                />
            </div>
            ...
            <div className="flex flex-wrap gap-2 align-items-center">
                <Dropdown
                    ...
                    options={gradeLevels}
                    onChange={(e) => setSelectedGradeLevel(e.value)}
                    placeholder="Groups"
                />
                <Dropdown
    ...
    return (
        <>
            ...
            <Dialog
                visible={newGradeDialog}
                style={{ width: "450px" }}
                header="Nueva Nota"
                modal
                className="p-fluid"
                onHide={() => setNewGradeDialog(false)}
            >
                <form action="" method="POST">
                    ...
                    <div className="p-dialog-footer flex justify-around">
                        <Button
                            type="button"
                            label="Cancelar"
                            className="bg-red-600 hover:bg-red-500 text-center px-6 py-2 text-white pl-4 p-button p-component p-button-outlined"
                            severity="danger"
                            icon="pi pi-times"
                            onClick={() => setNewGradeDialog(false)}
                        />
                        <Button
                            type="submit"
                            label="Aceptar"
                            className="bg-blue-600 hover:bg-blue-500 text-center px-6 py-2 text-white pl-4 p-button p-component"
                            icon="pi pi-check"
                        />
                    </div>
                </form>
            ...
            <Dialog
                visible={editGradeDialog}
                style={{ width: "450px" }}
                header="Editar Nota"
                modal
                className="p-fluid"
                onHide={() => setEditGradeDialog(false)}
            >
                <form action="" method="POST">
                    <div className="p-field">
                        <label>Student</label>
                    ...
                    <div className="p-field">
                        ...
                        <Dropdown
                            id="edit_grade_level"
                            value={editGradeData?.grade_level}
                            options={gradeLevels}
                            onChange={(e) =>
                                setEditGradeData({
                                    ...editGradeData,
                    ...
                    <div className="p-field">
                        ...
                        <Dropdown
                            id="edit_time_period"
                            value={editGradeData?.time_period}
                            options={timePeriods}
                            onChange={(e) =>
                                setEditGradeData({
                                    ...editGradeData,
                    ...
                    <div className="p-dialog-footer flex justify-around">
                        <Button
                            ...
                            icon="pi pi-times"
                            severity="danger"
                            onClick={() => setEditGradeDialog(false)}
                        />
                        <Button
                            type="submit"
                            label="Aceptar"
                ...
                <form action="" method="POST">
                    <div className="p-field flex mb-5 items-center">
                        ...
                        <p className="m-0 ml-3">
                            Seguro que desea eliminar la calificación del
                            estudiante
                        </p>
                    </div>
